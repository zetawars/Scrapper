...
namespace DataExtractor.Models
{
    ...
    //public async void ParisJob()
    //{
    //    string url = "https://www.parisjob.com/emplois/recherche.html";
    ...
    //    try
    //    {
    //        var list = document.QuerySelectorAll(".annonce").ToList();
    //        foreach (var element in list)
    //        {
    //            ParisJob parisJob = new ParisJob();
    //            try
    //            {
    //                var no_recruteur = element.QuerySelectorAll("h1").First().QuerySelectorAll("a").Last().Attributes.ToList();
    //                var data_attrib = no_recruteur.Where(x => x.Name == "data-click").ToList().First().Value;
    //                /*
    //                 {"name":"controleur-de-gestion",
    //                 "id":"2573186",
    //                 "price":"1",
    //                 "brand":"abylsen","category":"gestion",
    //                 "variant":"FORM_DO_CLASSIQUE","list":"LO-Moteur","position":"1","url":""}
    //                 */
    //                dynamic k = JsonConvert.DeserializeObject(data_attrib);
    //                try
    //                {
    //                    parisJob.brand = k.brand;
    //                    parisJob.brand = HttpUtility.HtmlDecode(parisJob.brand);
    //                    parisJob.brand = parisJob.brand.Trim();
    ...
    //                try
    //                {
    //                    parisJob.category = k.category;
    //                    parisJob.category = HttpUtility.HtmlDecode(parisJob.category);
    //                    parisJob.category = parisJob.category.Trim();
    ...
    //            try
    //            {
    //                var localisation = element.QuerySelectorAll(".picto-localisation-small").First().ParentNode.InnerText;
    //                parisJob.localisation = HttpUtility.HtmlDecode(parisJob.localisation);
    //                parisJob.localisation = localisation.Trim();
    ...
    public void InsertToDB(ParisJob cadrempoli)
    {
        using (SqlConnection con = new SqlConnection(@"Data Source=(local);Initial Catalog=Wines;Integrated Security=True"))
        {
            con.Open();
            string query = $@"INSERT INTO [dbo].[parisjob] ([brand] ,[category], [localisation]) VALUES ('{cadrempoli.brand.Replace("'", "''")}' ,'{cadrempoli.category.Replace("'", "''")}', '{cadrempoli.localisation.Replace("'", "''")}')";
    ...
    //    {
    //        con.Open();
    //        string query = $@"INSERT INTO [dbo].[cadrempoli] ([nom_recruteur] ,[localisation]) VALUES ('{cadrempoli.nom_recruteur.Replace("'", "''")}' ,'{cadrempoli.localisation.Replace("'", "''")}')";
    //        SqlCommand cmd = new SqlCommand(query, con);
    //        cmd.ExecuteNonQuery();
    //        con.Close();
    //    }
    ...
    //        string html = await GetHtml(newUrl);
    //        GetRequiredElementsFromHtml(html);
    //    }
    //    return;
    //}
